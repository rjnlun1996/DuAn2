...
public interface OrderRepository extends JpaRepository<Order, Integer>{
	...
	@Query("SELECT SUM(o.total) FROM Order o, StatusOrder so WHERE o.id = so.order.id AND o.enabled=1 AND so.enabled=1 AND so.current=1 AND so.status.id=?1")
	String calTotalRevenue(String status);
	
	@Query("SELECT SUM(od.quantity) FROM Order o, StatusOrder so, OrderDetail od WHERE o.id = so.order.id AND o.id = od.order.id AND o.enabled=1 AND so.enabled=1 AND od.enabled=1 AND so.current=1 AND so.status.id=?1")
	String calTotalProductSold(String status);
	
	@Query("SELECT COUNT(o) FROM Order o, StatusOrder so WHERE o.id = so.order.id AND o.enabled=1 AND so.enabled=1 AND so.current=1 AND so.status.id=?1")
	String calTotalOrder(String status);
	
	@Query("SELECT YEAR(o.createdAt) as yearReport, MONTH(o.createdAt) as monthReport, SUM(o.total) AS totalReport FROM Order o, StatusOrder so WHERE o.id = so.order.id AND o.enabled=1 AND so.enabled=1 AND so.current=1 AND so.status.id= 'DHT'  GROUP BY YEAR(o.createdAt), MONTH(o.createdAt) ORDER BY YEAR(o.createdAt) DESC, MONTH(o.createdAt) DESC")
	List<IReport> report();
